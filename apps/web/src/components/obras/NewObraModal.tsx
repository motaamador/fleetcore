...
import { createObra } from '@/app/dashboard/obras/actions'
import type { Currency, ProjectStatus } from '@fleetcore/types'
...
// ── Componente ───────────────────────────────────────────────────────────────
export function NewObraModal({ open, onClose, onSuccess }: NewObraModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        setErrors({ name: `Error al crear la obra: ${err.message}` })
      }
    })
  }
      ...
      <aside className="fixed inset-y-0 right-0 w-full max-w-lg bg-surface border-l border-border shadow-2xl z-50 flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-background">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-text-primary">Nueva Obra</h2>
              <p className="text-xs text-text-muted">Completa los datos del proyecto</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="px-6 py-4 border-t border-border bg-background flex items-center justify-end gap-3">
          ...
          >
            {isPending ? (
              ...
              <><HardHat className="w-4 h-4" /> Crear Obra</>
            )}
          </button>
        </div>
