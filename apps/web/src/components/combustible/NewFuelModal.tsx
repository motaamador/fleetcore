...
import { createFuelAction } from '@/app/dashboard/combustible/actions'
...
// ── Componente ─────────────────────────────────────────────────────────────────
export function NewFuelModal({ open, onClose, onSuccess, vehicles, drivers }: NewFuelModalProps) {
      ...
      <aside className="fixed inset-y-0 right-0 w-full max-w-lg bg-surface border-l border-border shadow-2xl z-50 flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-background">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-text-primary">Registrar Carga de Combustible</h2>
        ...
        {/* Formulario */}
        <form id="nuevo-combustible-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        ...
        <div className="px-6 py-4 border-t border-border bg-background flex items-center justify-end gap-3">
          ...
          <button form="nuevo-combustible-form" type="submit" disabled={isPending} className="btn-primary flex items-center gap-2 min-w-[150px] justify-center">
            {isPending
              ? <><Loader2 className="w-4 h-4 animate-spin" /> Guardando...</>
              : <><Fuel className="w-4 h-4" /> Registrar Carga</>
            }
          </button>
        </div>
